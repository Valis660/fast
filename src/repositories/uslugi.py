...
class RoomsUslugiRepository(BaseRepository):
    ...
    async def set_room_uslugi(self, room_id: int, uslugi_ids: list[int]) -> None:
        get_current_usliugi_ids_query = (
            select(self.model.uslugi_id)
        ...
        if ids_to_insert:
            insert_m2m_uslugi_stmt = (
                insert(self.model)
                .values([{"room_id": room_id, "uslugi_id": u_id} for u_id in ids_to_insert])
            )
